...
const OrderItem = () => {
    ...
    const decrementItemQuantity = useStore((store) => store.decrementItemQuantity)
    ...
    const handlePlaceOrder = async () => {
        try {
            ...
            const tableBookings = tableState.map((item: FLOOR_TABLES_TYPE) => [item.id, 60])
            if (Number(amount) < 0) {
                toast({
                    title: "Error",
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="p-3 bg-muted rounded-lg">
                            <div className="flex justify-between items-start">
                                ...
                                {
                                    !selectedUser ? <Link href={`/users/add`}>
                                        <Button size='sm' variant='outline' >
                                            <Plus />
                                        </Button>
                                    </Link> : <Button onClick={() => setSelectedUser(undefined)} size='icon' variant='destructive'>
                                        <X />
                                    </Button>
                                }
