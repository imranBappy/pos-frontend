...
export const wasteColumns: ColumnDef<WASTE_TYPE>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            if (responsibleUser) {
                return (
                    <Button variant={'link'}>
                        <Link href={`/users/${responsibleUser.id}`} className="capitalize">{responsibleUser.name}</Link>
                    </Button>
                );
            } else {
    ...
    },
    {
        accessorKey: "note",
        header: () => "Note",
        cell: ({ row }) => <div className="line-clamp-2">{row.getValue("note")}</div>, // Optionally truncate long notes
    },
    {
    ...
    },
    {
        id: "actions",
        enableHiding: false,
        cell: ({ row }) => <ActionsDropdown item={row.original} />, // Pass the waste object
    },
];
