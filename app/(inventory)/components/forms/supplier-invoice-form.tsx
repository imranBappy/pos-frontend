...
import { SUPPLIER_INVOICE_ITEM_QUERY, SUPPLIER_INVOICE_ITEMS_QUERY, SUPPLIER_INVOICE_QUERY, SUPPLIER_INVOICES_QUERY } from "@/graphql/supplier-invoice/queries"
import { SUPPLIER_INVOICE_ITEM_DELETE_MUTATION, SUPPLIER_INVOICE_ITEM_MUTATION, SUPPLIER_INVOICE_MUTATION } from "@/graphql/supplier-invoice/mutations"
...
export function SupplierInvoiceForm({ invoiceId }: { invoiceId?: string }) {
    ...
    const { data: invoiceRes, loading: invoiceQueryLoading } = useQuery(SUPPLIER_INVOICE_QUERY, {
    ...
    const { data: invoiceItemsRes, loading: invoiceItemsQueryLoading } = useQuery(SUPPLIER_INVOICE_ITEMS_QUERY, {
        ...
        onCompleted: (res) => {
            ...
            supplierInvoiceItems.forEach(({ node }: { node: SUPPLIER_INVOICE_ITEM_TYPE }) => {
                newItems.set(node.item.id, { id: node.id, quantity: node.quantity, price: Number(toFixed(node.price)) })
            })
            addItems(newItems)
        },
        skip: !invoiceId
    ...
    const [getInvoiceItem, { loading: invoiceItemQueryLoading }] = useLazyQuery(SUPPLIER_INVOICE_ITEM_QUERY)


    // Invoice Mutation
    const [invoiceMutation, { loading: create_loading }] = useMutation(SUPPLIER_INVOICE_MUTATION, {
        onCompleted(data) {
            setOrderId(data?.supplierInvoiceCud?.supplierInvoice.id)
        },
        refetchQueries: [{
            query: SUPPLIER_INVOICES_QUERY, variables: {
                offset: 0,
                first: 10,
                search: ""
            }
    ...
    const [invoiceItemMutation, { loading: itemCreate_loading }] = useMutation(SUPPLIER_INVOICE_ITEM_MUTATION)

    // delete invoice item
    const [deleteInvoiceItem, { loading: deleteInvoiceItemMutationLoading }] = useMutation(SUPPLIER_INVOICE_ITEM_DELETE_MUTATION)


    //  list of registered suppliers
    const { loading: supplier_loading, data: supplierRes, } = useQuery(SUPPLIERS_QUERY, {
        variables: {
            first: 100,
        },
        onError: (error) => {
    ...
    const suppliers: OPTION_TYPE[] = supplierRes?.suppliers?.edges?.map(({ node }: { node: SUPPLIER_TYPE }) => ({
        value: node.id,
        label: node.name,
    })) || [];
    ...
    async function onSubmit(data: InvoiceFormSchema) {
        try {
            ...
            if (!data.invoiceImage && !data.supplier && !imagePreviewUrl) {
                form.setError('supplier', { message: "Select supplier or Input invoice image" })
                return;
            }
            ...
            if (invoiceId) {
                ...
                if (invoiceRes?.supplierInvoice.status !== ORDER_STATUSES.PENDING) {
                    setWorning(`The invoice is updateable. Only panding invoice can update.`)
                    return 0;
                }
                ...
                for (const [itemId, value] of selectedItems) {
                    if (value.id) {
                        ...
                        const invoiceItemRes = await getInvoiceItem({ variables: { id: value.id } })

                        const preCurrentStock = itemRes?.data?.item?.currentStock
                        const preQuantity = invoiceItemRes.data.parchageInvoiceItem.quantity

                        const newCurrentStock = (preCurrentStock - preQuantity) + value.quantity
            ...
            if (data.invoiceImage && invoiceRes?.supplierInvoice?.invoiceImage) {
                // delete old image
                const deleted = await deleteImageFromS3(invoiceRes?.supplierInvoice.invoiceImage)
            ...
            const res = await invoiceMutation({
                variables: {
                    ...
                    invoiceImage: uploadedFiles ? uploadedFiles : invoiceRes?.supplierInvoice.invoiceImage,
                    id: invoiceId || undefined,
                    supplier: data.supplier || undefined,
                    status: PAYMENT_STATUSES.PENDING,
                    due: invoiceId ? toFixed(invoiceRes?.supplierInvoice?.due) : toFixed(amount)
                },
            })
            ...
            if (invoiceId) {
                ...
                invoiceItemsRes?.parchageInvoiceItems?.edges?.forEach(({ node }: { node: SUPPLIER_INVOICE_ITEM_TYPE }) => {
    ...
    const isLoading = deleteInvoiceItemMutationLoading || invoiceItemQueryLoading || itemQueryLoading || supplier_loading || loading || itemCreate_loading || invoiceQueryLoading || invoiceItemsQueryLoading
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <SwitchItem
                                    disabled={supplier_loading || Boolean(form.watch('invoiceImage'))}
                                    form={form}
                                    name="supplier"
                                    label="Supplier"
                                    options={suppliers}
                        ...
                        <button type="submit" disabled={create_loading || itemCreate_loading}>
